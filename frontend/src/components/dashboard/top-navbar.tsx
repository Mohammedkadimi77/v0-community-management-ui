...
export function TopNavbar() {
  ...
  const canCreatePost = hasFeatureAccess(user?.role, "create")
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-border bg-background/95 px-6 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      ...
      <div className="relative w-full max-w-md">
        ...
        <Input
          type="search"
          placeholder="Search posts, analytics, comments..."
      ...
      <div className="flex items-center gap-3">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-80">
            ...
            <DropdownMenuItem className="flex flex-col items-start gap-1 py-3">
              <span className="font-medium">Weekly analytics ready</span>
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="relative h-9 w-9 rounded-full">
              <Avatar className="h-9 w-9">
                <AvatarImage src={normalizeImageUrl(user?.avatar_url) ?? "/avatars/user.jpg"} alt="User" />
          ...
          <DropdownMenuContent align="end" className="w-56">
            <DropdownMenuLabel className="font-normal">
              <div className="flex flex-col space-y-1">
                <p className="text-sm font-medium">{`${user?.prenom ?? ""} ${user?.nom ?? ""}`.trim() || "Utilisateur"}</p>
